...
export const projects: Project[] = [
  {
    ...
    title: "bandstand",
    description:
      "Philadelphia jazz, tonight. Twenty-one rooms across the city: live jazz clubs, jam sessions, listening rooms, vinyl bars, DJ nights, and the bistros that play real jazz on the system. Per-venue scrapers (Chris' Jazz Cafe, South Jazz Kitchen) re-pull nightly, the rest hand-curated with confidence tags and verify-on-source links. Repertory-style editorial layout with a city map at /map.",
    longDescription:
      "Built because the Philly jazz calendar lives in twelve places at once: Chris' Jazz Cafe has the cleanest listings, Heritage updates on Instagram, La Rose is word of mouth, Solar Myth is on the Ars Nova page, and Time has a poster taped to the door. bandstand stitches them together, then extends the picture out past the stages to vinyl bars (Brewerytown Beats), DJ rooms (Trestle, Bok), and the restaurants whose music someone actually picked (Friday Saturday Sunday, Royal Boucherie, Hop Sing, Suraya, Vernick). Six tag categories, color-coded everywhere from the venue cards to the map markers. Visual design borrows from the repertory page in my lab portfolio: warm cream paper, ink black, signal red, Source Serif italic mastheads ('Tonight.', 'The Week.', 'The Map.'), JetBrains Mono caps for meta, paper-grain SVG overlay. Per-venue scrapers under src/scrapers/ run via `npm run refresh` and ship as part of the prebuild step on Vercel, so every deploy bakes in current data. A failed scraper keeps the previous batch instead of taking the build down. Chris' Jazz Cafe pulls 54 upcoming shows from the events page (cheerio against .event-list-item blocks, two-set nights collapsed to one card). South Jazz Kitchen reads the /jazz-club/ index and follows each /event/SLUG/ to parse the JSON-LD Event block — clean structured data with startDate/endDate/sameAs. Multi-day runs expand to one Occurrence per day. The schedule resolver walks any date range in America/New_York, merges scraped + hand-curated, dedupes by id, applies overrides for cancellations. Map page is MapLibre GL JS with openfreemap positron (no API key), markers colored by primary tag, filter chips that won't accept the empty set. Vercel Cron at /api/cron/refresh triggers a Deploy Hook nightly at 10 UTC. /tonight + /week + /sessions (jam filter, 2 weeks ahead) + /map + /venues + /venues/[slug] + /about. Stack: Next.js 16, React 19, TypeScript, Tailwind v4, maplibre-gl 5, cheerio for scraping. All static SSG except the cron route.",
  ...
  },
  {
    ...
    description:
      "Completion of Pulse Intelligence's Product Engineer take-home. Next.js + react-three-fiber scene that renders 31 drillholes and 14 mineralised intercepts from a real ASX gold drilling announcement, with click-to-inspect, source PDF deep links, a filterable hole list, a mobile bottom-sheet sidebar, and an auto-fitting camera. Deployed.",
    longDescription:
      "Pulse Intelligence's brief is a 4-6 hour take-home: turn an ASX drilling announcement into something a mining investor can walk through in 3D. The data is one 1.4MB PDF plus two CSVs (31 holes, 14 mineralised intercepts). Each collar carries lat/lon plus MGA51 east/north/rl in metres, a signed dip (negative = down), and an azimuth clockwise from north. The downhole direction vector falls out as (cos|dip|·sin az, cos|dip|·cos az, -sin|dip|), and a point at depth d is just collar + dir·d. I render in MGA51 metres directly, recentred at the mean collar position so the camera works with hundreds-of-metres offsets instead of MGA-zone absolutes, and override the camera's up vector to Z so vertical reads vertical despite Three's Y-up default. Each hole draws as a thin grey trace from collar to TD; each intercept is a coloured cylinder along its interval with grade mapped to a yellow-orange-red HSL ramp clamped at 20 g/t Au. The sidebar's default view is a filterable, intercept-aware sorted list of every hole; click a row (or tap a collar / intercept on the 3D side) to flip to the detail view with prospect, hole type, total depth, dip/azimuth, lat/lon + MGA51 coords, the full intercept list for the hole, and a deep link to the source PDF page that called out the selected intercept. Camera auto-fits via drei <Bounds>, with a reset button that re-fits to the full scene. Esc deselects. Mobile gets a three-state bottom-sheet sidebar that auto-pops to half on selection. 13 Vitest cases pin down the geometry (vertical / horizontal / mixed dips, dip-sign tolerance, origin recentre, gradeColor clamp). Standard library only on the data side: a 12-line CSV splitter, since the supplied files have no quoted commas and pulling in papaparse for 31 rows wouldn't earn its weight. NOTES.md walks the design picks (Bounds vs hand-computed camera, two-primitive vs textured tubes, no proj4, discrete vs continuous intercept ribbon) and what I'd add next (per-collar hover labels, synced PDF panel, section view, fly-to camera move).",
    tech: ["Next.js", "TypeScript", "react-three-fiber", "Three.js", "Tailwind", "Vitest"],
    github: "https://github.com/c-tonneslan/pulse-drillholes",
    live: "https://pulse-drillholes.vercel.app",
    status: "live",
    category: "fullstack",
  },
  {
